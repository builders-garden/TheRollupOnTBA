...
export const bullMeters = sqliteTable(
  "bull_meters",
  {
    id: text().primaryKey().notNull(),
  ...
  (table) => [index("idx_bull_meters_brand_id").on(table.brandId)],
);
...
export const user = sqliteTable(
  ...
  (table) => [uniqueIndex("user_farcaster_fid_unique").on(table.farcasterFid)],
);
...
export const wallet = sqliteTable(
  ...
  (table) => [index("idx_wallet_user_id").on(table.userId)],
);
