 ...
 * Brand table
 */
export const brandTable = sqliteTable("brands", {
  id: text("id")
    .primaryKey()
    .$defaultFn(() => ulid()),
  name: text("name"),
  ...
  description: text("description"),
  websiteUrl: text("website_url"),
  activePlugins: text("active_plugins", { mode: "json" }).$type<string[]>(),
  socialMediaUrls: text("social_media_urls", { mode: "json" }).$type<
    { platform: string; url: string; iconSlug: string }[]
  >(),
...
export type CreateBrand = typeof brandTable.$inferInsert;
export type UpdateBrand = Partial<CreateBrand>;

/**
 * Featured tokens table
 */
export const tokensTable = sqliteTable("featured_tokens", {
  id: text("id")
    .primaryKey()
    .$defaultFn(() => ulid()),
  brandId: text("brand_id")
...
export type FeaturedToken = typeof tokensTable.$inferSelect;
export type CreateFeaturedToken = typeof tokensTable.$inferInsert;
export type UpdateFeaturedToken = Partial<CreateFeaturedToken>;
...
export const tipsTable = sqliteTable("tips", {
  ...
  brandId: text("brand_id")
    .notNull()
    .references(() => brandTable.id, { onDelete: "cascade" }),
  payoutAddress: text("payout_address", { mode: "json" }).$type<Address>(),
  payoutBaseName: text("payout_base_name"),
  payoutEnsName: text("payout_ens_name"),
  amounts: text("amounts", { mode: "json" }).$type<number[]>(),
  createdAt: text("created_at").default(sql`CURRENT_TIMESTAMP`),
});
